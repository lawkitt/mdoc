...
- **Syntax highlighting** — fenced code with a language tag colors its tokens via
  a host-supplied `on_highlight` closure (bring your own engine; mdoc passes
...
Use the crate from this repository:

```toml
[dependencies]
mdoc-markdown = { path = "crates/mdoc-markdown" }
```
...
```rust
use std::rc::Rc;
use mdoc_markdown::{MarkdownView, MarkdownStyle};
...
`$…$` (`math_text`), typeset by a host renderer — and mdoc-style
`[[wiki-links]]` and `#tags`.
...
foldable `-`/`+` variant), mdoc-style `[[wiki-links]]` and `#tags`
