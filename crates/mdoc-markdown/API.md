...
The complete public API of [`mdoc-markdown`](README.md) — every exported item,
...
- **[Part I — `mdoc_markdown::syntax`](#part-i--mdoc_markdownsyntax)** — always
...
isn't public. Feature `—` = always compiled (`mdoc_markdown::syntax`);
...
# Part I — `mdoc_markdown::syntax`

Always compiled, **dependency-free** (no `gpui`, no `markdown`): the shared
construct recognition that this crate's reader, `mdoc-editor`'s WYSIWYG view,
...
- Rejected: `javascript:`, `data:`, `file:`, `smb:`, `vbscript:`, `mailto:`,
  ...
  empty string. A host that resolves local files (mdoc opens them in the PDF
...
isn't a PDF (`file.pdf#p3` keeps its page-jump meaning). A mdoc page title
...
(mdoc `[[Page::sub]]` links, `C++::method`) isn't mistaken for a property.
...
rounded chip). Both of mdoc's property panels render values through this so
...
mdoc-editor's WYSIWYG folds and self-heals — editing the heading drops the
...
| `line_height` | `f32` | Body line height as a multiple of `text_size`. Hosts with an editor match its ratio so reading and editing line up (default `1.45`, mdoc-editor's) |
