...
## DOCX comments (2026-09-19)

The converter's comment path did not retain the legal-opinion fixture's comment
text, despite painting a gray margin. Comments now travel as worker-produced JSON
metadata alongside the temporary PDF. A virtualized read-only panel shows author,
date, full plain text, and selected source text. It collapses to a count header.
Range extraction supports overlapping and cross-paragraph ranges; point comments
use paragraph context, and unanchored comments remain listed. Comment order follows
the comments part. Rich formatting and modern reply/resolution metadata are not
reproduced.

A temporary ZIP copy removes comment range/reference elements and empties the
comments part before PDF conversion, avoiding duplicate/broken converter margins.
Package validation still runs on the original snapshot first. Neither source
files nor imported Markdown are modified. Preview replacement/close owns the
panel together with the PDF, preserving the old comments on replacement failure.

`comments.docx`, generated by `generate.py`, qualifies Unicode, author/date,
multiple paragraphs, overlapping ranges, point comments, and unanchored comments.
Worker tests check the metadata, body text, rasterization, and source preservation;
GPUI tests check replacement failure, switching to PDF, closing, and unchanged
Markdown. `uploaded_comments_preserved` additionally checks the two supplied
comment-bearing documents and is ignored when the private fixtures are absent.
