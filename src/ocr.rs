...
fn validate(root: &Path) -> Result<Installed, String> {
    ...
    OarOcrEngine::from_models_with_runtime(&models, Some(&installed.onnx))
...
pub fn install() -> Result<Installed, String> {
    if !SUPPORTED {
        return Err("Local OCR is currently qualified on Apple Silicon macOS only.".into());
    }
    install_in(&root()?)
}
...
fn extract_runtime(root: &Path, runtime: &Runtime, archive: &File) -> Result<(), String> {
    ...
    for entry in tar.entries().map_err(|e| e.to_string())? {
        ...
        let destination = if path.ends_with(Path::new("lib").join(runtime.library))
...
mod tests {
    ...
    fn runtime_extraction_ignores_debug_files_and_symlinks() {
        ...
        extract_runtime(dir.path(), &RUNTIMES[0], &archive).unwrap();
        assert_eq!(
    ...
    #[test]
    #[ignore = "downloads OCR runtimes/models; requires Apple Silicon macOS"]
